...
export default function Header() {
      ...
      <>
        {
          data.map((value, index)=>
            <div key={index} className="group">
              <Link
                href={value.href}
                className='text-base font-medium relative transition-colors duration-300 hover:text-gray-700'
              >
                {value.name}
      ...
      <>
        {
          data.map((value, index)=>
            <DropdownMenuItem key={index}>
              <Link href={value.href} className='text-base font-medium transition-colors duration-200 hover:text-gray-700'>{value.name}</Link>
            </DropdownMenuItem>
          )
        }
      </>
  ...
  return (
    <header className='bg-white h-24 w-full mx-auto py-6 flex items-center justify-center sticky top-0 z-50 shadow-sm'>
      <nav className="flex w-11/12 max-w-[1240px] justify-between gap-4 lg:gap-10 items-center">
        <div className="md:hidden">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button type='button' className='text-2xl font-bold rotate-90 transition-transform duration-300 hover:scale-110 active:scale-95'>|||</button>
            </DropdownMenuTrigger>
        ...
        <div className="transition-transform duration-300 hover:scale-105">
          <Link href='/' className='text-3xl font-bold mr-auto transition-colors duration-300 hover:text-gray-700'>
            SHOP.CO
          </Link>
        </div>
        ...
        <div className="gap-3 max-h-full items-center flex">
          ...
          <div className="transition-transform duration-300 hover:scale-110 active:scale-95">
            <Link href='/cart' className="transition-opacity duration-300 hover:opacity-70">
          ...
          <div className="transition-transform duration-300 hover:scale-110 active:scale-95">
            <Link href="/profile" className="transition-opacity duration-300 hover:opacity-70">
              <svg
                width="20px"
                height="20px"
                viewBox="0 0 24 24"
